import type { TopFitResult, TopDimension } from "@/lib/tops";
...
const ORDER: TopDimension[] = ["chest", "shoulder", "sleeve", "length", "neck"];
...
export function TopResultRow({
  ...
  const { garment: g, bestSize, score, verdict, note } = result;
    ...
    <li className="grid grid-cols-[2.5rem_1fr] items-start gap-x-5 border-t border-line py-7 last:border-b sm:grid-cols-[3rem_1fr_auto] sm:gap-x-8">
      ...
      <div className="min-w-0">
        ...
        <p className="mono mt-1 text-xs uppercase tracking-wider text-faint">
          {g.fitType}
          {g.stretchPct ? ` · ${g.stretchPct}% stretch` : ""}
          {g.provenance === "estimated" ? " · est. specs" : " · measured"}
        </p>
